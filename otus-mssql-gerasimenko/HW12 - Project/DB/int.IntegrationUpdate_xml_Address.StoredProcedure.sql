USE [GDS2]
GO
/****** Object:  StoredProcedure [int].[IntegrationUpdate_xml_Address]    Script Date: 24.07.2023 19:48:20 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [int].[IntegrationUpdate_xml_Address] (
	@PACKNUM varchar(50),
	@SenderCode varchar(150),
	@PacketUpdateRows int output
)
AS BEGIN
...
JOIN [match].[Address] AS b ON a.[Uid] = b.TargetValue
WHERE a.PROCESS_READ IS NULL
...
BEGIN TRY
	...
	--[dict].[Addresse]
	update upd
	set	Street = isnull(a.Street, NULL)
	...
	JOIN [match].[Address] AS c ON a.[Uid] = c.TargetValue
	JOIN [dict].[Addresse] as upd ON c.LocalValue = upd.id
	...
	SET PROCESS_READ = getdate()
		,PROCESS_COMMENT = 'Запись обновлена'
	FROM [int].[xml_Address] AS a
	JOIN @CHECKED AS b ON a.[Uid] = b.ExternalCode

	SELECT @PacketUpdateRows = COUNT(*) FROM @CHECKED
	SELECT @PacketUpdateRows
END TRY
BEGIN CATCH
	IF @@trancount > 0
